...
/**
 * Hostgroupsummary is a custom model we use to add more columns than the IcingaDB Hostgroupsummary
 */
class Hostgroupsummary extends UnionModel
{
    ...
    public function getColumns(): array
    {
        return [
            ...
            'hosts_downtime_handled' => new Expression(
                'SUM(CASE WHEN host_state != 1 AND host_in_downtime = \'y\' THEN 1 ELSE 0 END)'
            ),
            'hosts_downtime_active' => new Expression(
                'SUM(CASE WHEN host_in_downtime = \'y\' THEN 1 ELSE 0 END)'
            ),
            'services_downtime_handled' => new Expression(
                'SUM(CASE WHEN service_state != 1 AND service_in_downtime = \'y\' THEN 1 ELSE 0 END)'
            ),
